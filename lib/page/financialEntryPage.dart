...
class _MyHomePageState extends State<FinancialEntryPage> {
  FinancialEntryRegister register = FinancialEntryRegister();
  ...
  void changeType() async{
    try{
      typeList.clear();
      if(_isCredit){
        typeList = _account_credit!.map((element)=> element.description).toList();
      }
      else{
         typeList = _account_debit!.map((element)=> element.description).toList();
      }   
      setState(() {
